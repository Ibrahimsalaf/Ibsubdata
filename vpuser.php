...
include_once(ABSPATH .'wp-content/plugins/vtupress/functions.php');
  ...
  elseif(vp_getoption("vprun") != "none"){
    ...
    $title = "Service Currently Inactive";
    $message = "You are using an incompatible php version of <b>$phpVersion</b>, this might break up your experience. Kindly send a request to your developer or change your php version yourself on cpanel if you use one to <b>7.4</b> and not lesser or higher than that version ";
    dump_error($title, $message);
  }
...
$vpaccess = vp_getuser($ma_id,'vp_user_access');

$my_current_plan = vp_getuser($ma_id,'vr_plan');
...
if(isset($_SESSION["user_id"]) && isset($_SESSION["add_unrecorded"])){
	if(is_numeric($_SESSION["user_id"]) && $_SESSION["add_unrecorded"] == 'yes' && isset($_SESSION["recipient"]) && isset($_SESSION["service"])){
  global $wpdb;
  
  $service = $_SESSION["service"];
  $trans_id = $_SESSION["request_id"];
  $serv_tab = $wpdb->prefix.$service;
  $service_result = $wpdb->get_results("SELECT * FROM $serv_tab WHERE request_id = '$trans_id'");
  
  if(empty($service_result) || $service_result == null){
  $vptable = $wpdb->prefix."vp_transactions";
  $vpresult = $wpdb->get_results("SELECT * FROM $vptable WHERE request_id = '$trans_id'");
...
//////////////////
 ...
 if(strtolower($vpaccess) == "false" || strtolower($vpaccess) == "access"){
